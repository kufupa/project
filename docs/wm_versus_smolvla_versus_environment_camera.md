...
- Default sim path in [`project/src/segment_grpo_loop.py`](../src/segment_grpo_loop.py) `rollout_with_chunks`: **jepa-parity** RGB (`render_jepa_rgb`) feeds WM scoring, goal-latent fallback from live frame, and comparison-strip “real” rows. **SmolVLA** chunk sampling uses a **policy** image derived by default as **horizontal flip** of that stream (`_derive_policy_rgb_for_smolvla`) so pixels match best_video **V+H** corner2 contract while WM stays **V-only**. Disable with `--no-smolvla-policy-hflip-corner2`. Chunk exploration noise defaults to **0** (`--smolvla-noise-std`); set `>0` to restore stochastic per-timestep jitter. Legacy obs-based RGB remains available via `--no-wm-sim-camera-parity`.
...
- **CLI:** `scripts/run_segment_grpo.py` — `--wm-goal-hflip` / `--no-wm-goal-hflip`, `--wm-sim-camera-parity` / `--no-wm-sim-camera-parity`, `--wm-sim-img-size` (default 224), `--smolvla-policy-hflip-corner2` / `--no-smolvla-policy-hflip-corner2`, `--smolvla-noise-std`. [`scripts/segment_grpo/run_first_episode_real_pipeline.sh`](../scripts/segment_grpo/run_first_episode_real_pipeline.sh) forwards WM parity defaults for Slurm.
